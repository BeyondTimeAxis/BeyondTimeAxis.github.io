...
function logscript_init() {
    const logscript_oldproceed = window.proceed;

    let logscript_lastText = "";
    let logscript_lastName = "";
    let logscript_lastFrameID = -1;
    let logscript_lastMerged = false;

    let logscript_currentFrameData = false;
    let logscript_currentText = "";
    let logscript_currentName = "";
    let logscript_currentFrameID = -1;
    let logscript_mergedFrame = false;

    let logscript_log = [];

    window.proceed = function(conditions, backwards){
        ...
        if (ls_clean_text_contents1 === ls_clean_text_contents2 || logscript_lastMerged){
            if (!logscript_mergedFrame
                && logscript_currentFrameID !== logscript_lastFrameID
                && (player_status.proceed_click_met
                    || (player_status.proceed_timer_met && player_status.proceed_timer && player_status.proceed_typing)
                    || player_status.proceed_typing_met
                    || (!player_status.proceed_click_met && !player_status.proceed_timer_met && !player_status.proceed_typing_met))
                && logscript_currentText !== ""){
...
if (document.readyState !== 'complete') {
    addEventListener('load', logscript_init);
} else {
    logscript_init();
}